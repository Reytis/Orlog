...
import { baldrInvulnerability, bragiVerve, brunhildFury, freyjaPlenty, freyrGift, friggSight, heimdallWatch, helGrip, idunnRejuvenation, lokiTrick, mimirWisdom, odinSacrifice, skadiHunt, skuldClain, thorStrike, thrymrTheft, tyrPledge, ullrAim, varBond, vidarMight } from "./favors.js";
import { v4 as uuidv4 } from "uuid";
//Select Randomly the player who play first
export function pileOuFace(arr) {
...
export function favorTwoApplication(Caller, Target) {
    let favor = Caller.selectedFavor;
    let caller = Object.assign({}, Caller);
    let target = Object.assign({}, Target);
...
export function sortResultDices(face, second) {
    let n = 0;
...
export function convertPlayerToPlayerObject(player) {
    return {
        name: player.name,
...
export const isMyTurn = (id) => {
    return localStorage.getItem('myTurnId') === id;
};
export const setMyTurnId = (id) => {
    localStorage.setItem('myTurnId', id);
};
export const resetMyTurnId = (id) => {
    if (isMyTurn(id)) {
