...
import { GameStates } from '../types.js';
import { canChooseGuard, canDropGuard, canFavorOneGuard, canFavorTwoGuard, canNextTurnGuard, canSelectDiceGuard, canSelectFavorGuard, canSelectGuard, canSetUpGuard, canStartGuard, canThrowGuard, canToResoluteGuard, canWinGuard } from './guards.js';
import { chooseDiceAction, chooseFavorAction, dropPlayerAction, favorOneResAction, favorTwoResAction, leaveAction, nextTurnAction, pointResAction, restartAction, resultResAction, selectDiceAction, selectFavorAction, setUpAction, startAction, throwDiceAction } from './actions.js';
import { interpret } from 'xstate';
// Define a model for the game state, with context and events
export const GameModel = createModel({
    players: [], // Array of players in the game
    curentThrower: null, // The player currently throwing dice
    mainPlayer: null, // The main player, e.g., the current turn player
}, {
    events: {
        // Game start event
        start: () => ({}),
        // Event to set up the game with player information
        setUpGame: (playerId, playerName, playerCharacter, playerFavor, playerIsReady, playerCount, playerResult, playerStat, playerDice) => ({ playerId, playerName, playerCharacter, playerFavor, playerIsReady, playerCount, playerResult, playerStat, playerDice }),
        dropPlayer: (playerId) => ({ playerId }),
        // Player action events (e.g., throwing dice, selecting, etc.)
        throwDices: (playerId) => ({ playerId }),
        selectDices: (playerId) => ({ playerId }),
        chooseDice: (playerId, dice) => ({ playerId, dice }),
        chooseFavor: (playerId, selectedFavor) => ({ playerId, selectedFavor }),
        selectFavor: (playerId, selectedFavor, level, sacrifice) => ({ playerId, selectedFavor, level, sacrifice }),
        // Game flow events
        toResolute: () => ({}),
        pointRes: () => ({}),
        favorOneRes: () => ({}),
...
export function makeGame(state = GameStates.LOBBY, context = {}) {
    const machine = interpret(GameMachine.withContext(Object.assign(Object.assign({}, GameModel.initialContext), context // Merge with any passed-in context
