...
import { damageDealt, favorOneApplication, favorTwoApplication, lockingRes, pileOuFace, ppTheft, throwDice } from "../func/gameFunc.js";
// Action to set up a player in the game
export const setUpAction = (context, event) => ({
...
export const startAction = (context) => {
...
export const throwDiceAction = (context, event) => {
...
export const selectDiceAction = (context, event) => {
    const player = context.players.find(p => p.id === event.playerId); // Find the player by ID
...
export const chooseDiceAction = (context, event) => {
    const player = context.players.find(p => p.id === event.playerId); // Find the player by ID
...
export const selectFavorAction = (context, event) => {
    var _a;
    const player = context.players.find(p => p.id === event.playerId); // Find the player by ID
    // Update the player's favor selection
    const updatedFavor = (_a = player.favor) === null || _a === void 0 ? void 0 : _a.map((f, index) => {
        if (f !== undefined && f.selected !== false) {
            ...
            if (f.selected === true && index !== event.selectedFavor) {
                return Object.assign(Object.assign({}, f), { selected: false, sacrifice: event.sacrifice });
            }
            else if (f.selected === true && index === event.selectedFavor && f.level !== event.level) {
                return Object.assign(Object.assign({}, f), { level: event.level, sacrifice: event.sacrifice });
            }
        }
        // Select or deselect favor
        if (index === event.selectedFavor) {
            return Object.assign(Object.assign({}, f), { selected: !f.selected, level: event.level, sacrifice: event.sacrifice });
        }
        return f;
    });
    const updatedPlayer = Object.assign(Object.assign({}, player), { favor: updatedFavor });
    const updatedPlayers = context.players.map((p) => p.id === event.playerId ? updatedPlayer : p);
    return {
...
export const chooseFavorAction = (context, event) => {
    const player = context.players.find(p => p.id === event.playerId); // Find the player by ID
...
export const pointResAction = (context) => {
...
export const favorOneResAction = (context) => {
    var _a, _b;
    ...
    if (mainP.selectedFavor && otherP.selectedFavor) {
        if (((_a = mainP.selectedFavor) === null || _a === void 0 ? void 0 : _a.priority) >= ((_b = otherP.selectedFavor) === null || _b === void 0 ? void 0 : _b.priority)) {
...
export const resultResAction = (context) => {
    let mainP = context.players.find(p => p.id === context.mainPlayer); // Main player
...
export const favorTwoResAction = (context) => {
    var _a, _b;
    let mainP = context.players.find(p => p.id === context.mainPlayer); // Main player
    let otherP = context.players.find(p => p.id !== context.mainPlayer); // Opponent player
...
export const nextTurnAction = (context) => {
    var _a, _b;
    let mainP = context.players.find(p => p.id === context.mainPlayer); // Main player
    let otherP = context.players.find(p => p.id !== context.mainPlayer); // Opponent player
    // Reset PP and PV updates for both players
    ...
    (_a = otherP.favor) === null || _a === void 0 ? void 0 : _a.forEach(f => f !== undefined ? f.selected = false : f);
    (_b = mainP.favor) === null || _b === void 0 ? void 0 : _b.forEach(f => f !== undefined ? f.selected = false : f);
...
export const restartAction = (context) => {
    let mainP = context.players.find(p => p.id === context.mainPlayer); // Main player
    let otherP = context.players.find(p => p.id !== context.mainPlayer); // Opponent player
    // Reset PP and PV updates for both players
    mainP.stats.pp.update = 0;
...
export const dropPlayerAction = (context, event) => {
