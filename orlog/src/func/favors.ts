...
export function lokiTrick(level: number, targetDices: Dice[]) {
    ...
    let bannedDice: Dice[] = [] // Array to hold banned dice
...
export function freyjaPlenty(level: number) {
    let spentPP = 0 // Power Points cost
    let addDice: Dice[] = [] // Array to hold added dice
...
export function friggSight(level: number, playerDice: Dice[]) {
    ...
    let rerolled: number[] = [] // Track rerolled dice to avoid rerolling the same die
...
export function skadiHunt(level: number, playerDice: Player, targetDices: Player) {
    ...
    let bonus: Player["bonus"] = [{ type: Face.bow, num: level, multiply: true }] // Bonus configuration
...
export function vidarMight(level: number, targetRes: Player) {
    let spentPP = 0 // Power Points cost
    let newRes: Player['lockRes'] = lockingRes(targetRes) // Locked resources of the target
    let bonus: Player["bonus"] = [{type: Face.helmet, num: -level * 2, multiply: false}] // Helmet reduction bonus
...
export function brunhildFury(level: number, targetRes: Player) {
    let spentPP = 0 // Power Points cost
    let newRes: Player['lockRes'] = lockingRes(targetRes) // Locked resources of the target
    let bonus: Player["bonus"] = [{type: Face.axe, num: level === 1 ? 1.5 : level, multiply: true}] // Axe increase bonus
...
export function baldrInvulnerability(level: number, targetRes: Player) {
    let spentPP = 0 // Power Points cost
    let newRes: Player['lockRes'] = lockingRes(targetRes) // Locked resources of the target
    let bonus: Player["bonus"] = [{type: Face.shield, num: 0, multiply: true},{type: Face.helmet, num: 0, multiply: true}] // Shield and helmet bonus
...
export function ullrAim(level: number, targetRes: Player) {
    let spentPP = 0 // Power Points cost
    let newRes: Player['lockRes'] = lockingRes(targetRes) // Locked resources of the target
    let bonus: Player["bonus"] = [{type: Face.shield, num: 0, multiply: false}] // Shield reduction bonus
