...
describe("machine", () => {
  ...
  describe("SetUp & Start", () => {
    ...
    it('should set a player', () => {
      expect(machine.send(GameModel.events.setUpGame('1', 'Marco', undefined, undefined, false, 0, [],{pv: {current:15, update:0}, pp:{current:0, update:0}}, undefined)).changed).toBe(true)
      expect(machine.state.context.players).toHaveLength(1)
    })
    it('should not set a player twice or have more than 2 players', () => {
      expect(machine.send(GameModel.events.setUpGame('1', 'Marco', undefined, undefined, false, 0, [], {pv: {current:15, update:0}, pp:{current:0, update:0}}, undefined)).changed).toBe(true)
      expect(machine.send(GameModel.events.setUpGame('1', 'Marco', undefined, undefined, false, 0, [], {pv: {current:15, update:0}, pp:{current:0, update:0}}, undefined)).changed).toBe(false)
      expect(machine.send(GameModel.events.setUpGame('2', 'Reytis', undefined, undefined, false, 0, [], {pv: {current:15, update:0}, pp:{current:0, update:0}}, undefined)).changed).toBe(true)
      expect(machine.send(GameModel.events.setUpGame('3', 'Porco', undefined, undefined, false, 0, [], {pv: {current:15, update:0}, pp:{current:0, update:0}}, undefined)).changed).toBe(false)
    })
  ...
  describe("Turn (throwing, validating, choosefavor)", () => {
    ...
    it('should add selected dice to the result of the player an change current player if count equal to 3 select all not selected dice', () => {
      ...
      expect(machine.state.context.players.find(p => p.id === '1')!.result.length === 6).toBe(true)
